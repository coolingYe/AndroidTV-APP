...
public final class ApkUtil {
    ...
    public static boolean installApkSession(Context context, String apkFilePath, IntentSender statusReceiver) {
        ...
        try {
            ...
            addApkToInstallSession(apkFilePath, session, file.length());
    ...
    private static void addApkToInstallSession(String assetName, PackageInstaller.Session session, long lengthBytes)
            throws IOException {
        // It's recommended to pass the file size to openWrite(). Otherwise installation may fail
        // if the disk is almost full.
        try (OutputStream packageInSession = session.openWrite("package", 0, lengthBytes);
    ...
    public static int getAppVersionCode(Context context, String packageName) {
        PackageManager manager = context.getPackageManager();
        try {
            return manager.getPackageInfo(packageName, PackageManager.GET_CONFIGURATIONS).versionCode;
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
        }
        return 0;
