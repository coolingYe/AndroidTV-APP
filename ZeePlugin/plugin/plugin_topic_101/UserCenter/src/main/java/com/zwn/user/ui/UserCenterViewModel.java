...
public class UserCenterViewModel extends BaseViewModel {
    ...
    public MutableLiveData<UserInfoResp> userInfo = new MutableLiveData<>();
    public MutableLiveData<LoadState> userInfoReqState = new MutableLiveData<>();

    public MutableLiveData<CreateSessionForUserInfoResp> userInfoSession = new MutableLiveData<>();
    public MutableLiveData<LoadState> userInfoSessionReqState = new MutableLiveData<>();
    public MutableLiveData<SessionResultStatus> userInfoSessionResult = new MutableLiveData<>();
    ...
    public void createUserInfoSession() {
        userInfoSessionReqState.setValue(LoadState.Loading);
        mUserRepository.createSessionForUserInfo()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(this)
                .subscribe(new DisposableObserver<BaseResp<CreateSessionForUserInfoResp>>() {
                    @Override
                    public void onNext(BaseResp<CreateSessionForUserInfoResp> t) {
                        if (t.code == BaseConstants.API_HANDLE_SUCCESS) {
                            if (t.data != null && t.data.sessionId != null && !t.data.sessionId.isEmpty()) {
                                userInfoSession.setValue(t.data);
                                userInfoSessionReqState.setValue(LoadState.Success);
                            } else {
                                pToast.setValue("会话创建失败！");
                                userInfoSessionReqState.setValue(LoadState.Failed);
                            }
                        } else {
                            pToast.setValue(t.message);
                            userInfoSessionReqState.setValue(LoadState.Failed);
                        }
                    }
    ...
    public void reqUserInfoSessionResult(String sessionId) {
        userInfoSessionResult.setValue(SessionResultStatus.LOADING);
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("sessionId", sessionId);
        mUserRepository.getSessionResultForUserInfo(jsonObject)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(this)
                .subscribe(new DisposableObserver<BaseResp<JsonElement>>() {
                    @Override
                    public void onNext(BaseResp<JsonElement> t) {
                        switch (t.code) {
                            case BaseConstants.API_HANDLE_SUCCESS:
                                userInfoSessionResult.setValue(SessionResultStatus.SUCCESS);
                                break;
                            case 10011:
                                pToast.setValue(t.message);
                                userInfoSessionResult.setValue(SessionResultStatus.TIMEOUT);
                                break;
                            case 10012:
                                pToast.setValue(t.message);
                                userInfoSessionResult.setValue(SessionResultStatus.DISABLED);
                                break;
                        }
                    }
                    ...
                    @Override
                    public void onError(Throwable e) {
                        userInfoSessionResult.setValue(SessionResultStatus.FAILED);
                    }
