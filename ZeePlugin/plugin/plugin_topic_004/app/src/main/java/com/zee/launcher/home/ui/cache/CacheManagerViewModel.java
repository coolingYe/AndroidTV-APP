...
public class CacheManagerViewModel extends BaseViewModel {
    ...
    public MutableLiveData<DataLoadState<List<ProductListMo.Record>>> mldAllProductRecodeListLoadState = new MutableLiveData<>();
    ...
    public void reqAllProductListBySkuIds(List<String> skuIdList) {//for api permission filter
        mldAllProductRecodeListLoadState.setValue(new DataLoadState<>(LoadState.Loading));
        DataRepository.getInstance().getProductListBySkuIds(new ProductListBySkuIdsReq(skuIdList, CommonUtils.getDeviceSn()))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(CacheManagerViewModel.this)
                .subscribe(new DisposableObserver<BaseResp<List<ProductListMo.Record>>>() {
                    ...
                    public void onNext(@NonNull BaseResp<List<ProductListMo.Record>> response) {
                        if (response.code == BaseConstants.API_HANDLE_SUCCESS) {
                            if (response.isCache) {
                                mldToastMsg.setValue("数据获取失败，请检查网络状态");
                                mldAllProductRecodeListLoadState.setValue(new DataLoadState<>(LoadState.Failed));
                            } else {
                                mldAllProductRecodeListLoadState.setValue(new DataLoadState<>(LoadState.Success, response.data));
                            }
                        } else {
                            mldToastMsg.setValue(response.message);
                            mldAllProductRecodeListLoadState.setValue(new DataLoadState<>(LoadState.Failed));
    ...
    public void getDownloadList() {
        List<DownloadInfo> allDownloadInfo = CareController.instance.getAllDownloadInfo(
                "type=" + BaseConstants.DownloadFileType.PLUGIN_APP + " AND extraId IS NOT NULL");
