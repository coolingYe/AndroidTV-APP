...
public class DetailActivity extends BaseActivity implements View.OnFocusChangeListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        downloadPro.setTypeface(FontUtils.typefaceMedium);
        ...
        TextView txtDetailGuessLike = findViewById(R.id.txt_title_type_list);
        txtDetailGuessLike.setTypeface(FontUtils.typefaceMedium);
    ...
    private void initClickListener() {
        ...
        ImageView imgLikeDetail = findViewById(R.id.img_like_detail);
        imgLikeDetail.setOnClickListener(v -> {
        ...
        imgCollectDetail.setNextFocusRightId(R.id.img_collect_detail);
        imgLikeDetail.setOnFocusChangeListener(this);
        imgLikeDetail.setNextFocusUpId(R.id.img_wifi_top_bar);
        txtProductSummaryDetail.setNextFocusUp(findViewById(R.id.top_bar_view).findViewById(R.id.img_wifi_top_bar));
        txtProductSummaryDetail.setNextFocusLeft(rlVideoRoot);
...
//                    String fileSize = FileUtils.FormatFileSize(Long.parseLong(publishResp.getPackageSize()));
                    ...
                    downloadPro.setTextColor(getColor(R.color.detail_download_text_color));
    ...
    private void updateDownloadTip(int progress) {
        ...
        gradientProgressViewDetail.setProgress(progress);
        if (CommonUtils.isTopic008Enable()) {
            downloadPro.setTextColor(0xFF607D28);
            return;
        }
        if (progress >= 32) {
            downloadPro.setTextColor(0xFFFFFFFF);
        } else downloadPro.setTextColor(0xFFFC5A39);
    }
    ...
    private void showUpgradeDialog() {
        ...
        upgradeDialog.setCancelText("继续体验");
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        switch (v.getId()) {
            case  R.id.txt_product_summary_detail:
                LinearLayout llDetailSummary = findViewById(R.id.ll_product_summary_detail);
                if (hasFocus) {
                    llDetailSummary.setBackgroundResource(R.drawable.shape_detail_summary_frame);
                } else {
                    llDetailSummary.setBackgroundResource(R.drawable.selector_transparent_bg);
                }
                break;
            case  R.id.rl_video_root:
                FrameLayout frameLayout = findViewById(R.id.fl_product_image);
                if (hasFocus) {
                    CommonUtils.scaleView(frameLayout, 1.04f);
                } else {
                    v.clearAnimation();
                    CommonUtils.scaleView(frameLayout, 1f);
                }
                break;
            case R.id.layout_download_detail:
            case  R.id.img_like_detail:
            case  R.id.img_collect_detail:
                if (hasFocus) {
                    CommonUtils.scaleView(v, 1.07f);
                } else {
                    v.clearAnimation();
