...
import static com.zeewain.base.utils.CommonUtils.isTopic008Enable;
...
public class MainActivity extends BaseActivity implements ViewTreeObserver.OnGlobalFocusChangeListener {
    ...
    private ConstraintLayout llMainTabRoot;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        viewPageMain.setOffscreenPageLimit(1);
        ...
        llMainTabRoot = mainTabView.findViewById(R.id.ll_main_tab_root);
    ...
    private void initChannel() {
        if (isTopic008Enable()) {
            TopBarView topBarView = findViewById(R.id.top_bar_view);
            topBarView.setImgUser(R.mipmap.top_bar_user_login);
            topBarView.setUserInfoPadding(40);

            llMainTabRoot.findViewById(R.id.v_main_tab_bg).setBackgroundResource(R.drawable.shape_c16_83a04c);
        } else {
            llMainTabRoot.findViewById(R.id.v_main_tab_bg).setBackgroundResource(R.mipmap.img_main_tab_bg);
        }
    }

    private void initView(GlobalLayout globalLayout) {

        if (globalLayout.global.themeName != null) {
            SPUtils.getInstance().put(SharePrefer.themeName, globalLayout.global.themeName);
        }
        TopBarView topBarView = findViewById(R.id.top_bar_view);
        topBarView.updateUserImg(getThemeName(SPUtils.getInstance().getString(SharePrefer.themeName, ""),
                HostManager.getHostSpString(SharePrefer.userAccount, "用户ID")));
        ...
        if (globalLayout.layout.basic.pageHeaderLayout != null && globalLayout.layout.basic.pageHeaderLayout.config != null) {
            if (globalLayout.layout.basic.pageHeaderLayout.config.showLogo) {
                ...
                topBarView.setShowTxtLogo(false);
            }
        }
    ...
    private void switchTableEvent(int position) {
        if (CommonUtils.isTopic903Enable()) {
            TopBarView topBarView = findViewById(R.id.top_bar_view);
            topBarView.changeUserInfoForTopic903(position, viewModel.globalLayout.layout.pages.get(position).config.fullName);
        } else if (CommonUtils.isTopic009Enable()) {
            TopBarView topBarView = findViewById(R.id.top_bar_view);
            topBarView.changeUserInfoForTopic009(position, viewModel.globalLayout.layout.pages.get(position).config.fullName);
        }
    }
    ...
    public void shiftPages(int position) {
        mainTabAdapter.setFocusedPosition(position);
        recyclerViewMainTab.requestFocus();
    }
    ...
    public void onGlobalFocusChanged(View view, View view1) {
        Log.d("test", "onGlobalFocusChanged newFocus: " + view1);
        Log.d("test", "onGlobalFocusChanged oldFocus: " + view);
    }
