...
public class TopBarView extends ConstraintLayout implements View.OnFocusChangeListener {
    ...
    private void initView(Context context) {
        ...
        txtUserInfo.setBackgroundResource(isTopic008Enable() ? R.drawable.shape_c16_83a04c : R.drawable.gradient_h_c16_f7d100_ef4e12);
    ...
    public void setUserInfoPadding(int left) {
        txtUserInfo.setPadding(DisplayUtil.dip2px(txtUserInfo.getContext(), left), 0, DisplayUtil.dip2px(txtUserInfo.getContext(), 13), 0);
    }

    public void changeUserInfoForTopic903(int position, String targetInfo) {
        if (position == 0) {
            setUserInfoPadding(13);
            setImgUser(0);
        } else {
            setUserInfoPadding(42);
            if (targetInfo.contains("跃动")) {
                setImgUser(R.mipmap.img_user_info_1);
            } else setImgUser(R.mipmap.img_user_info_2);
        }
        updateUserImg(targetInfo);
    }

    public void changeUserInfoForTopic009(int position, String targetInfo) {
        if (position == 0) {
            setUserInfoPadding(13);
            setImgUser(0);
        } else {
            ...
            switch (targetInfo) {
                case "娱乐":
                    setImgUser(R.mipmap.icon_tab_1);
                    break;
                case "康体":
                    setImgUser(R.mipmap.icon_tab_2);
                    break;
                case "乐学":
                    setImgUser(R.mipmap.icon_tab_3);
                    break;
                case "养生":
                    setImgUser(R.mipmap.icon_tab_4);
                    break;
            }
        }
        updateUserImg(targetInfo);
