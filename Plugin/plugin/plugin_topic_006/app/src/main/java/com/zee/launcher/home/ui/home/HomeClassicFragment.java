...
import static com.zee.launcher.home.config.ProdConstants.CategoryIconMap;
...
public class HomeClassicFragment extends Fragment {
    ...
    private void initView() {
        ...
        for (int i = 0; i < pageLayoutDTO.appCardListLayoutList.size(); i++) {
            ...
            ProductListType productListType = new ProductListType(ProductListType.TYPE_CLASSIC_MODULE, mCategoryIndex + "_" + i, getCategoryFullName(appCardListLayout), getCategoryIcons(appCardListLayout));
            productListType.appCardListLayout = appCardListLayout;
            if (productListType.appCardListLayout.config.appSkus.size() > 0)
                typeList.add(productListType);
        }

        recyclerViewHomeClassic.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL,false));
    }
    ...
    private String getCategoryFullName(AppCardListLayout appCardListLayout) {
        if (appCardListLayout.config.title.isEmpty()) {
            return pageLayoutDTO.config.fullName;
        }
        return appCardListLayout.config.title;
    }

    private String getCategoryName(AppCardListLayout appCardListLayout) {
        if (appCardListLayout.config.title.isEmpty()) {
            return pageLayoutDTO.name;
        }
        return appCardListLayout.config.title;
    }

    private int getCategoryIcons(AppCardListLayout appCardListLayout) {
        Integer iconId = CategoryIconMap.get(getCategoryName(appCardListLayout));
    ...
    public void checkReqDataDone() {
        ...
        if (newPendingCount <= 0) {
            ...
            for (int i = 0; i < typeList.size(); i++) {
                ProductListType productListType = typeList.get(i);
                if ((productListType.type == ProductListType.TYPE_BANNER && hasBannerAPP) || productListType.type == ProductListType.TYPE_CLASSIC_MODULE) {
                    if (!mViewModel.isExistCacheProductRecodeList(productListType.careKey)) {
    ...
    public void updateUiByDataReady() {
        ...
        recyclerViewHomeClassic.setAdapter(productListAdapter);
        if (verticalItemDecoration == null) {
            verticalItemDecoration = new VerticalItemDecoration(DisplayUtil.dip2px(recyclerViewHomeClassic.getContext(), 15), 0,
                    DisplayUtil.dip2px(recyclerViewHomeClassic.getContext(), 8));
            recyclerViewHomeClassic.addItemDecoration(verticalItemDecoration);
        }
    }
