...
package com.zee.launcher.home.widgets.curl;
...
public class CurlView extends GLSurfaceView implements View.OnTouchListener,
	...
	public void setPageChanged(PageChanged mPageChanged) {
		this.mPageChanged = mPageChanged;
	}
	...
	public void onDrawFrame() {
		...
		int currentIndexTmp = mCurrentIndex;
		...
		if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
			...
			mAnimate = false;
			requestRender();
			if(currentIndexTmp != mCurrentIndex && mPageChanged != null){
				mPageChanged.onPageChanged(mCurrentIndex);
			}
		} else {
	...
	public boolean onTouchAction(TouchAction touchAction) {
		// No dragging during animation at the moment.
		// TODO: Stop animation on touch event and return to drag mode.
		if (mAnimate || mPageProvider == null) {
			return false;
		...
		// Store pointer position.
		mPointerPos.mPos.set(touchAction.getX(), touchAction.getY());
		mRenderer.translate(mPointerPos.mPos);
		if (mEnableTouchPressure) {
			mPointerPos.mPressure = touchAction.getPressure();
		} else {
			mPointerPos.mPressure = 0.8f;
		}
