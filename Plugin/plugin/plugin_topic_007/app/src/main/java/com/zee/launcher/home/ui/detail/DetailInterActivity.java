...
import com.zee.launcher.home.ui.loading.LoadingPluginInterActivity;
...
public class DetailInterActivity extends DetailBaseActivity implements View.OnFocusChangeListener {
    ...
    private final DownloadListener downloadListener = new DownloadListener() {
        ...
        public void onFailed(String fileId, int type, int code) {
            if (fileId.equals(currentFileId)) {
                ...
                runOnUiThread(() -> {
                    if (!NetworkUtil.isNetworkAvailable(DetailInterActivity.this)) {
                        showToast("网络连接异常！");
                    }
                });
            }
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detail_inter);

        skuId = getIntent().getStringExtra("skuId");
        packageName = getIntent().getStringExtra(BaseConstants.EXTRA_PACKAGE_NAME);
        if (TextUtils.isEmpty(skuId) || TextUtils.isEmpty(packageName)) {
            finish();
            return;
        }
...
//        imgBack.setNextFocusLeftId(com.zeewain.base.R.id.img_top_bar_back);
        initClickListener();
        initViewObservable();
    ...
    private void initClickListener() {
        ...
        layoutDownloadDetail.setNextFocusDownId(R.id.layout_download_detail);
        ...
        imgBack.setOnKeyListener((v, keyCode, event) -> {
            if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT && event.getAction() == KeyEvent.ACTION_DOWN) {
                v.clearAnimation();
                v.startAnimation(AnimationUtils.loadAnimation(DetailInterActivity.this, com.zeewain.base.R.anim.host_shake));
            } else if (keyCode == KeyEvent.KEYCODE_DPAD_UP && event.getAction() == KeyEvent.ACTION_DOWN) {
                v.clearAnimation();
                v.startAnimation(AnimationUtils.loadAnimation(DetailInterActivity.this, com.zeewain.base.R.anim.host_shake_y));
            }
            return false;
        });
...
//                handleGuessLikeUpdate();
        ...
        detailViewModel.mCheckRemoteCameraState.observe(this, isExist -> {
            ...
            } else startActivityForResult(new Intent(this, UnConnectActivity.class),REQUEST_CODE_APP);
        });
    ...
    private void handleDownload() {
        if (downloadBinder != null) {
            DownloadInfo dbDownloadInfo = CareController.instance.getDownloadInfoByFileId(currentFileId);
            if (dbDownloadInfo == null) {
                DownloadInfo downloadInfo = DownloadHelper.buildDownloadInfo(getUseContext(), detailViewModel.proDetailResp, detailViewModel.publishResp, packageName+";");
                downloadInfo.extraId = "";
                downloadBinder.startDownload(downloadInfo);
            } else {
                handleSlaveMasterDescUpdate(packageName, dbDownloadInfo);
    ...
    private void startLoadingApplication(int resultCode) {
        ...
        if (akSkInfoString != null && !akSkInfoString.isEmpty()) {
            ...
            if (akSkResp != null) {
                ...
                intent.setClass(this, LoadingPluginInterActivity.class);
        ...
        public void handleMessage(Message msg) {
            ...
            if (activity != null) {
                if (msg.what == MSG_DOWNLOAD_ON_PROGRESS) {
                    ...
                    ((DetailInterActivity) activity).updateDownloadTip(progress);
                } else if (msg.what == MSG_DOWNLOAD_ON_FAILED) {
                    ((DetailInterActivity) activity).updateDownloadTipOnFailed();
                } else if (msg.what == MSG_DOWNLOAD_ON_UPDATE) {
                    ((DetailInterActivity) activity).updateDownloadTip();
                }
            }
        }
    }
