...
public class CurveView extends View {
   ...
   private int mLength = 30;
     ...
     * 获取屏幕的宽度
     */
    private  int mScreenWidth = getScreenWidth(getContext());
    /**
     * 获取实际屏幕的宽度
     */
     ...
     */
//    private  int mHeight = getScreenHeight(getContext()) - dp2px(getContext(), 190);
...
//    private final float mSideLength = dp2px(getContext(), 20);
    private final float mSideLength = dp2px(getContext(), 10);
    /**
     * X轴底部文字的高度
     */
//    private final int mXTextHeight = dp2px(getContext(), 30);
    private final int mXTextHeight = dp2px(getContext(), 0);
    ...
    public void setData(List<String> xAxis, List<String> yAxis) {
        ...
        for (int i = 0; i < xAxis.size(); i++) {
           ...
           // dataValue.add(new Pair<>(xAxis.get(i), (yHeight - value / 200f * yHeight)));
            dataValue.add(new Pair<>(xAxis.get(i), (yHeight - value / 320f * yHeight)));
        }
        invalidate();
        requestLayout();
    }
    ...
    private void initYtext() {
       ...
       // mYTextPaint.setTextSize(dp2px(getContext(), 12));
        mYTextPaint.setTextSize(dp2px(getContext(), 9));
    ...
    private void initInCircle() {
       ...
       // mInCirclePaint.setStrokeWidth(dp2px(getContext(), 2));
        mInCirclePaint.setStrokeWidth(dp2px(getContext(), 1));
    ...
    private void initBrokenLine() {
        ...
        mBrokenLinePaint.setFilterBitmap(true);
        mBrokenLinePaint.setColor(Color.parseColor("#644FFF"));

       // mBrokenLinePaint.setStrokeWidth(dp2px(getContext(), 2f));
        mBrokenLinePaint.setStyle(Paint.Style.STROKE);
        mBrokenLinePaint.setStrokeWidth(dp2px(getContext(), 0.5f));
    ...
    private void initShadowPaint() {
        ...
        mShadowPaint.setAntiAlias(true);
         LinearGradient shader = new LinearGradient(0, 0, 0, getMeasuredHeight(), new int[]{Color.parseColor("#DFD9FF"), Color.parseColor("#FFFFFF")}, new float[]{0.01f,  0.95f}, Shader.TileMode.REPEAT);
//        Shader shader = new LinearGradient(getWidth() / 2f, getHeight(), getWidth() / 2f, 0,
//                Color.parseColor("#DFD9FF"), Color.parseColor("#FFFFFF"), Shader.TileMode.CLAMP);
    ...
    private void drawCircleLine(Canvas canvas) {
        ...
        for (int i = 0; i < dataValue.size(); i++) {
            ...
            if ( i%5==0){
                // 绘制节点外圆
                mOutCirclePaint.setColor(Color.parseColor("#644FFF"));
                mOutCirclePaint.setStyle(Paint.Style.STROKE);
                canvas.drawCircle(mSideLength + mSpaceLength * i, pair.second-outRadius/4, outRadius/4, mOutCirclePaint);
            }
    ...
    private void drawShadow(Canvas canvas) {
        ...
        if (isCurve) {//曲线
            ...
            for (int i = 0; i < points.size() - 1; i++) {
                ...
                path.cubicTo(point3.x, point3.y, point4.x, point4.y, pEnd.x, pEnd.y);
                //减去文字和指示标的高度
             //   path.lineTo(pEnd.x, getHeight() - mXTextHeight-100);
                path.lineTo(pEnd.x, mHeight-mXTextHeight);
             //   path.lineTo(pStart.x, getHeight() - mXTextHeight-100);
                path.lineTo(pStart.x, mHeight-mXTextHeight);
            }
            path.close();
